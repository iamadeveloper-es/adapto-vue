...
4. **Capa de orquestación/agentes**: qué subagentes tienen sentido desde el día uno (por ejemplo, revisión de código, documentación, testing) y el criterio de cuándo deben correr en paralelo vs secuencial, priorizando coste/claridad sobre granularidad excesiva.

## Separación de ámbito
Aplica la misma convención que en otros proyectos: separa lo global de lo específico mediante una estructura de carpetas dentro de `.claude/skills/` y `.claude/agents/` (ej. `skills/generic/` vs `skills/project/`), reforzada con una convención fija en el front-matter o cabecera de cada archivo indicando su ámbito.
